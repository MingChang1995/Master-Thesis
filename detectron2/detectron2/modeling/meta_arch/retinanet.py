...
# A section has been added here to penalise overlap generated between bounding boxes.
...
class RetinaNet(DenseDetector):
    ...
    def forward_training(self, images, features, predictions, gt_instances):
        ...
        # If the penalty overlaps
        return self.newlosses(images, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes, features, predictions)
    ...
    def newlosses(self, images,  anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes, features, predictions):
        ...
        loss_overlap = self.overlap_loss(images, features, predictions)

        return {
            "loss_cls": loss_cls / self.loss_normalizer,
            "loss_box_reg": loss_box_reg / self.loss_normalizer,
            "loss_overlap": 50 * loss_overlap / self.loss_normalizer,
        }
    ...
    def overlap_loss(self, images: ImageList, features: List[Tensor], predictions: List[List[Tensor]] ):
        loss_overlap = 0
        result = self.forward_inference(
            images, features, predictions
        )
        for re in result:
              Bounding_boxex = re.pred_boxes
              Box = Bounding_boxex.tensor
              i = random.randint(0,len(Box)-2)
              box = Box[i]
              for j in range(i+1, len(Box)):
                box_j = Box[j]
                i_loss = self.iou_loss(box, box_j, reduction = "sum")
                loss_overlap += i_loss
        return loss_overlap
    
    def iou_loss(self, boxes1: torch.Tensor, 
             boxes2: torch.Tensor, 
             reduction: str = "none", 
