...
def batched_softer_nms(
    boxes: torch.Tensor, scores: torch.Tensor, alphas: torch.Tensor, idxs: torch.Tensor, iou_threshold: float
):
    """
    Same as torchvision.ops.boxes.batched_nms, but safer.
    """
    ...
    for id in torch.jit.annotate(List[int], torch.unique(idxs).cpu().tolist()):
        mask = (idxs == id).nonzero().view(-1)
        keep = softer_nms(boxes[mask], scores[mask], alphas[mask], iou_threshold, 0.3, 0.7, 2, 0.5)
        result_mask[mask[keep]] = True
    keep = result_mask.nonzero().view(-1)
    keep = keep[scores[keep].argsort(descending=True)]
    return keep
...
def soft_nms(boxes: torch.Tensor, scores: torch.Tensor, soft_threshold=0.3,  iou_threshold=0.7, weight_method=2, sigma=0.5):
    keep = []
    idxs = scores.argsort()
    while idxs.numel() > 0:
...
def softer_nms(boxes: torch.Tensor, scores: torch.Tensor, alphas: torch.Tensor, soft_threshold=0.3,  iou_threshold=0.7, weight_method=2, sigma=0.5):
    keep = []
    idxs = scores.argsort()
    while idxs.numel() > 0:
        ...
        if weight_method == 1:
            ...
            ge_threshod_idxs = idxs[ge_threshod_bool]
            scores[ge_threshod_idxs] *= (1. - ious[0][ge_threshod_bool])
            boxes_overlap = boxes[ge_threshod_idxs]
            alphas_overlap = alphas[ge_threshod_idxs]
            boxes_new = torch.cat(boxes[max_score_index], boxes_overlap, dim=0)
            alphas_new = torch.cat(alphas[max_score_index], alphas_overlap, dim=0)
            boxes[max_score_index] = (boxes_new * alphas_new).sum() / alphas_new.sum()
